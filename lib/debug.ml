...
let _int =
  P (fun inp -> match parse (pchar '-') inp with
...
let _float =
  let _float_full = 
    ...
    let* d = pchar '.' in
    let* f = _floatpart in
    P (fun inp -> Some((i,(String.make 1 d)^f), inp))
    ||>
    (fun (i,f) -> float_of_int i |> (+.) (float_of_string f))
  in
...
and _sub inp = 
  parse (_infix (parser _term) "-" (parser _expr) ||> make_sub <|> parser _term) inp
and _term inp =
